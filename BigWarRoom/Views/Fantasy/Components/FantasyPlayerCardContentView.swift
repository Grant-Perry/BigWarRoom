...
struct FantasyPlayerCardMainContentView: View {
    ...
    var body: some View {
        HStack(spacing: 8) { // 🔥 FIX: Reduced spacing from 12 to 8 to give score area more room
            ...
            VStack(alignment: .trailing, spacing: 4) {
                ...
                .padding(.bottom, 36)
                .padding(.trailing, 8) // 🔥 FIX: Reduced from 12 to 8 to give more space
            }
            .zIndex(3)
        }
    }
    ...
    private var scoreText: some View {
        Text(player.currentPointsString)
            ...
            .lineLimit(1)
            .minimumScaleFactor(0.7) // 🔥 FIX: Increased from 0.6 to 0.7 for better readability
            .scaleEffect(isPlayerLive ? (glowIntensity > 0.5 ? 1.15 : 1.0) : 1.0)
            .shadow(color: .black.opacity(0.9), radius: 3, x: 0, y: 2)
            .padding(.horizontal, 6) // 🔥 FIX: Reduced from 8 to 6 to give more space for text
            .padding(.vertical, 4)
            .frame(minWidth: 60) // 🔥 FIX: Ensure minimum width for the score area
            .overlay(
                onScoreTap != nil ? 
