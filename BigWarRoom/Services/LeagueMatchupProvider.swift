...
final class LeagueMatchupProvider {
    ...
    private func fetchSleeperData() async {
        ...
        // 🔥 PERF: Parallelize independent pre-reqs. We still must wait for rosters/users before
        // processing matchups because matchup processing needs manager names + roster->owner mapping.
        async let _ = fetchSleeperWeeklyStats()
        async let _ = fetchSleeperUsersAndRosters()
        _ = await ((), ())
